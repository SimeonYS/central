...
class CentralPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `central`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM central WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO central (date, title, link, content)"
